...
def plotgraphs(conn,tickers,imagepath):
    for ticker in tickers:
        ...
        plt.plot(pd.to_datetime(df['Date']).dt.date,df['SMA50'], color = 'cornflowerblue', label = 'SMA50')
        #plt.plot(pd.to_datetime(df['Date']).dt.date,df['TEMA5'], color = 'green', label = 'TEMA5')
        plt.plot(pd.to_datetime(df['Date']).dt.date,df['TEMA20'], color = 'red', label = 'TEMA20')
        plt.xticks(rotation=45)       
        plt.legend(loc="upper left")

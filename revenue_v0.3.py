...
sqlshortlist = """SELECT Symbol FROM _yf_financials_revenue_scores WHERE `Net Income` > 1000000 ORDER BY NetIncomePctChange DESC LIMIT 30"""
...
for symbol in shortlist:
    ...
    df = pd.DataFrame(sql_query, columns=['Date','Ticker','Adj Close','Close','High','Low','Open','Volume','Percent Change','SMA50','SMA150','SMA200','BB_low','BB_mid','BB_up'])#.tail(50)
    print(symbol[0])
    print(df.iloc[-1])

...
class SMACrossClose():
    ...
    def buy(self):
        ...
        if len(self.stock.df) > 70:
            if (self.stock.df['SMA50'].iloc[-9] - self.stock.df['Close'].iloc[-9] > 0) and \
                ((self.stock.df['SMA50'].iloc[-9] - self.stock.df['Close'].iloc[-9]) > (self.stock.df['SMA50'].iloc[-7] - self.stock.df['Close'].iloc[-7])) and \
                (self.stock.df['SMA50'].iloc[-1] - self.stock.df['Close'].iloc[-1] < 0) and \
                ((self.stock.df['SMA50'].iloc[-3] - self.stock.df['Close'].iloc[-3]) < (self.stock.df['SMA50'].iloc[-1] - self.stock.df['Close'].iloc[-1])):
                return 1
            else:
                return 0
        else:
